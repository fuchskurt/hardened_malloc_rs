...
fn update_submodules() {
    ...
    println!("Running command: \"{} {}\" in directory: {:?}",
             program,
             args.join(" "),
             current_dir(),);
    ...
    match ret.map(|status| (status.success(), status.code())) {
        ...
        Ok((false, Some(exit_code))) => {
            panic!("updating submodules failed with error code {}", exit_code)
        }
        Ok((false, None)) => {
...
fn check_compiler(compiler: &'static str) -> &'static str {
    ...
    match compiler_ret.map(|status| (status.success(), status.code())) {
        ...
        Ok((false, None)) => {
            panic!("compiler check exited with no error code, possibly killed by system")
        }
...
fn main() {
    ...
    let variant = if cfg!(feature = "light") { "light" } else { "default" };

    let build_args: Vec<String> = if cfg!(feature = "static") {
        vec![format!("VARIANT={}", variant),
    ...
    let make_output = make_command.current_dir("src/hardened_malloc/")
    ...
    if !make_output.status.success() {
        panic!("building hardened_malloc failed:\n{:?}\n{}\n{}",
               make_command,
    ...
    if cfg!(feature = "static") {
        ...
        let ar_output =
            ar_command.args(ar_args).output().unwrap_or_else(|error| {
                                                 panic!("Failed to run '{ar_command:?}': {error}");
                                             });

        if !ar_output.status.success() {
            panic!("creating static lib of hardened_malloc failed:\n{:?}\n{}\n{}",
                   ar_command,
